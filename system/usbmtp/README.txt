...
^^^^^^^^^^^^^^^
  ...
  add-on, your board-specific logic must provide the function:
    ...
    void board_usbmtp_initialize(void);

  This function will be called by the system/usbmtp indirectly via the
  ...
  implementation of board_usbmtp_initialize() see
  configs/mcu123-lpc124x/src/up_usbmtp.c or
  configs/stm3210e-eval/src/usbmtp.c
  ...
  CONFIG_SYSTEM_USBMTP_DEVMINOR1
    The minor device number of the block driver for the first LUN. For
    example, N in /dev/mmcsdN.  Used for registering the block driver. Default
    is zero.
  CONFIG_SYSTEM_USBMTP_DEVPATH1
    The full path to the registered block driver.  Default is "/dev/mmcsd0"
  CONFIG_SYSTEM_USBMTP_DEVMINOR2 and CONFIG_SYSTEM_USBMTP_DEVPATH2
    Similar parameters that would have to be provided if CONFIG_SYSTEM_USBMTP_NLUNS
    is 2 or 3.  No defaults.
  CONFIG_SYSTEM_USBMTP_DEVMINOR3 and CONFIG_SYSTEM_USBMTP_DEVPATH3
    Similar parameters that would have to be provided if CONFIG_SYSTEM_USBMTP_NLUNS
    is 3.  No defaults.
  CONFIG_SYSTEM_USBMTP_DEBUGMM
  ...
  CONFIG_SYSTEM_USBMTP_TRACEINIT
    Show initialization events
  CONFIG_SYSTEM_USBMTP_TRACECLASS
    Show class driver events
  CONFIG_SYSTEM_USBMTP_TRACETRANSFERS
    Show data transfer events
  CONFIG_SYSTEM_USBMTP_TRACECONTROLLER
    Show controller events
  CONFIG_SYSTEM_USBMTP_TRACEINTERRUPTS
