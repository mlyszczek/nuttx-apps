...
static void write_create(lv_obj_t * parent)
{
  ...
  lv_obj_set_size(ta, lv_page_get_scrl_width(parent), lv_obj_get_height(
                    parent) / 2);
  ...
  style_kb_rel.body.main_color      = LV_COLOR_HEX3(0x333); /*Recommended if
                                                             * LV_VDB_SIZE ==
                                                             * 0 and bpp > 1
                                                             * fonts are
                                                             * used*/
...
static lv_res_t keyboard_open_close(lv_obj_t * text_area)
{
  ...
  lv_obj_t * parent = lv_obj_get_parent(lv_obj_get_parent(ta)); /*Test area is
                                                                 * on the
                                                                 * scrollable
                                                                 * part of the
                                                                 * page but we
...
static void list_create(lv_obj_t * parent)
{
  ...
  lv_mbox_add_btns(mbox, mbox_btns, NULL);      /*The default action is
                                                 * close*/
...
static void chart_create(lv_obj_t * parent)
{
  ...
  lv_obj_align(slider, chart, LV_ALIGN_OUT_BOTTOM_MID, 0,
               (LV_VER_RES - chart->coords.y2 - lv_obj_get_height(slider)) /
               2);                                                                                                            /*Align
                                                                                                                               * to
                                                                                                                               * below
                                                                                                                               * the
  ...
  slider_action(slider);            /*Simulate a user value set the refresh
                                     * the chart*/
}
